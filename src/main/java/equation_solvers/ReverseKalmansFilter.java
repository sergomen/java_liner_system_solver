 ...
 */
public class ReverseKalmansFilter {
	...
	public State next(RowState rawState) {

		if (covariation_matrix == null) {
			covariation_matrix = Matrix.matrix_reverse(new double[][]{
		...
		double[][] assessment_state;
		if (previous_step_state[0][0] == 0)
			assessment_state = Matrix.matrix_multiplication(covariation_matrix, new double[][]{
				{rawState.velocity},
				{rawState.angle},
				{rawState.noise}
			});
		else
			assessment_state = Matrix.matrix_multiplication(covariation_matrix, previous_step_state);
